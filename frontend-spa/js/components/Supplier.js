const Supplier = {

data() {
    return {

        supplier: [],

        nama_supplier: '',
        alamat: '',
        telepon: '',

        editMode: false,
        editId: null

    }
},

mounted() {

    this.loadData();

},

methods: {

    async loadData() {

        try {

            const response =
                await api.get('/supplier');

            this.supplier = response.data;

        } catch(error) {

            console.log(error);

        }

    },

    async simpan() {

        if(this.nama_supplier.trim() == ''){

            alert('Nama supplier tidak boleh kosong');

            return;

        }

        try{

            if(this.editMode){

                await api.put(

                    `/supplier/${this.editId}`,

                    {

                        nama_supplier:
                        this.nama_supplier,

                        alamat:
                        this.alamat,

                        telepon:
                        this.telepon

                    }

                );

                alert('Supplier berhasil diupdate');

            }else{

                await api.post(

                    '/supplier',

                    {

                        nama_supplier:
                        this.nama_supplier,

                        alamat:
                        this.alamat,

                        telepon:
                        this.telepon

                    }

                );

                alert('Supplier berhasil ditambahkan');

            }

            this.resetForm();

            this.loadData();

        }catch(error){

            console.log(error);

            alert('Terjadi kesalahan');

        }

    },

    edit(item){

        this.editMode = true;

        this.editId = item.id;

        this.nama_supplier =
            item.nama_supplier;

        this.alamat =
            item.alamat;

        this.telepon =
            item.telepon;

        window.scrollTo({

            top:0,

            behavior:'smooth'

        });

    },

    async hapus(id){

        if(

            !confirm(
                'Yakin ingin menghapus supplier ini?'
            )

        ){

            return;

        }

        try{

            await api.delete(

                `/supplier/${id}`

            );

            alert('Supplier berhasil dihapus');

            this.loadData();

        }catch(error){

            console.log(error);

            alert('Gagal menghapus data');

        }

    },

    resetForm(){

        this.nama_supplier='';

        this.alamat='';

        this.telepon='';

        this.editMode=false;

        this.editId=null;

    }

},

template:`

<div class="min-h-screen bg-gradient-to-br from-blue-600 via-indigo-600 to-purple-700 py-10">

<div class="container mx-auto px-5">

    <!-- HEADER -->

    <div class="bg-white rounded-3xl shadow-xl p-8 mb-8">

        <h1 class="text-4xl font-extrabold text-gray-800">

            🚚 Data Supplier

        </h1>

        <p class="text-gray-500 mt-2">

            Kelola seluruh data supplier barang.

        </p>

    </div>

    <!-- FORM -->

    <div class="bg-white rounded-3xl shadow-xl p-8 mb-8">

        <h2 class="text-2xl font-bold mb-6 text-gray-700">

            {{ editMode ? '✏️ Edit Supplier' : '➕ Tambah Supplier' }}

        </h2>

        <div class="grid md:grid-cols-3 gap-4">

            <input

                v-model="nama_supplier"

                type="text"

                placeholder="Nama Supplier"

                class="border-2 border-gray-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"

            >

            <input

                v-model="alamat"

                type="text"

                placeholder="Alamat"

                class="border-2 border-gray-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"

            >

            <input

                v-model="telepon"

                type="text"

                placeholder="Nomor Telepon"

                class="border-2 border-gray-300 rounded-xl p-3 focus:outline-none focus:ring-2 focus:ring-blue-500"

            >

        </div>

        <div class="mt-6">

            <button

                @click="simpan"

                class="bg-green-500 hover:bg-green-600 text-white font-bold px-8 py-3 rounded-xl shadow"

            >

                {{ editMode ? '💾 Update Supplier' : '➕ Tambah Supplier' }}

            </button>

        </div>

    </div>

    <!-- TABEL -->

    <div class="bg-white rounded-3xl shadow-xl p-8">

        <h2 class="text-2xl font-bold mb-6 text-gray-700">

            📋 Daftar Supplier

        </h2>

        <div class="overflow-x-auto">

        <table class="w-full">

            <thead>

                <tr class="bg-blue-600 text-white">

                    <th class="p-3 rounded-l-xl">
                        No
                    </th>

                    <th class="p-3">
                        Nama Supplier
                    </th>

                    <th class="p-3">
                        Alamat
                    </th>

                    <th class="p-3">
                        Telepon
                    </th>

                    <th class="p-3 rounded-r-xl">
                        Aksi
                    </th>

                </tr>

            </thead>

            <tbody>

                <tr

                    v-for="(item,index) in supplier"

                    :key="item.id"

                    class="border-b hover:bg-gray-100 transition"

                >

                    <td class="p-3 text-center">

                        {{ index+1 }}

                    </td>

                    <td class="p-3">

                        {{ item.nama_supplier }}

                    </td>

                    <td class="p-3">

                        {{ item.alamat }}

                    </td>

                    <td class="p-3">

                        {{ item.telepon }}

                    </td>

                    <td class="p-3 text-center">

                        <button

                            @click="edit(item)"

                            class="bg-yellow-500 hover:bg-yellow-600 text-white px-4 py-2 rounded-lg mr-2"

                        >

                            ✏️ Edit

                        </button>

                        <button

                            @click="hapus(item.id)"

                            class="bg-red-500 hover:bg-red-600 text-white px-4 py-2 rounded-lg"

                        >

                            🗑️ Hapus

                        </button>

                    </td>

                </tr>

                <tr
                    v-if="supplier.length==0"
                >

                    <td
                        colspan="5"
                        class="text-center p-6 text-gray-500"
                    >

                        Belum ada data supplier.

                    </td>

                </tr>

            </tbody>

        </table>

        </div>

    </div>

    <!-- FOOTER BUTTON -->

    <div class="mt-8 flex justify-between">

        <router-link

            to="/dashboard"

            class="bg-gray-800 hover:bg-black text-white px-6 py-3 rounded-xl shadow"

        >

            ⬅️ Dashboard

        </router-link>

    </div>

</div>

</div>

`
}